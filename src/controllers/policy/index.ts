...
import { addEditPolicySchema, getPolicyByTypeSchema } from '../../validation';

export const add_policy = async (req, res) => {
    reqInfo(req)
    try {
        const { error, value } = addEditPolicySchema.validate(req.body || {});
        if (error) return res.status(HTTP_STATUS.BAD_REQUEST).json(new apiResponse(HTTP_STATUS.BAD_REQUEST, error.details[0].message, {}, {}));

        let response = await updateData(policyModel, { type: value.type }, value, { upsert: true });
        if (!response) return res.status(HTTP_STATUS.CONFLICT).json(new apiResponse(HTTP_STATUS.CONFLICT, `${value.type} policy already exists. Use edit instead.`, {}, {}));
